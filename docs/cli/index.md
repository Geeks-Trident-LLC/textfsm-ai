...
`PERPLEXITY_API_KEY`, `AZURE_OPENAI_API_KEY`) > `providers.yaml`.
...
| Option | Purpose |
|---|---|
| `--provider` (required) | Provider name: `openai`, `anthropic`, `gemini`, `deepseek`, `groq`, `xai`, `together`, `fireworks`, `cerebras`, `perplexity`, `azure` |
